...
public class Calculations
{
	...
	{
		if (block == null) return allBlocks;
		...
		GetAjacentBlocks(world, allBlocks, world.getBlockAt(block.getX() + 1, block.getY(), block.getZ()), countdown - 1);
		GetAjacentBlocks(world, allBlocks, world.getBlockAt(block.getX() - 1, block.getY(), block.getZ()), countdown - 1);
		GetAjacentBlocks(world, allBlocks, world.getBlockAt(block.getX(), block.getY() + 1, block.getZ()), countdown - 1);
		GetAjacentBlocks(world, allBlocks, world.getBlockAt(block.getX(), block.getY() - 1, block.getZ()), countdown - 1);
		GetAjacentBlocks(world, allBlocks, world.getBlockAt(block.getX(), block.getY(), block.getZ() + 1), countdown - 1);
		GetAjacentBlocks(world, allBlocks, world.getBlockAt(block.getX(), block.getY(), block.getZ() - 1), countdown - 1);
	...
	public static byte[] Obfuscate(ChunkInfo info)
	{
		...
		if(info.data.length == 2048 * (5 * info.chunkSectionNumber + info.extraSectionNumber) + 256 && OrebfuscatorConfig.getUseCache())
		{
			...
			//Chunk has changed, notify neighboring chunks
			if(storedHash != 0L)
			{
				(new ObfuscatedCachedChunk(cacheFolder, info.chunkX + 1, info.chunkZ, initialRadius, OrebfuscatorConfig.getUseProximityHider())).Invalidate();
				(new ObfuscatedCachedChunk(cacheFolder, info.chunkX - 1, info.chunkZ, initialRadius, OrebfuscatorConfig.getUseProximityHider())).Invalidate();
				(new ObfuscatedCachedChunk(cacheFolder, info.chunkX, info.chunkZ + 1, initialRadius, OrebfuscatorConfig.getUseProximityHider())).Invalidate();
				(new ObfuscatedCachedChunk(cacheFolder, info.chunkX, info.chunkZ - 1, initialRadius, OrebfuscatorConfig.getUseProximityHider())).Invalidate();
			}
		        ...
		        for (int y = 0; y < 16; y++)
		        {
			        for (int z = 0; z < 16; z++)
			        {
				        ...
				        for (int x = 0; x < 16; x++)
				        {
							...
							{
								proximityBlocks.add(info.player.getWorld().getBlockAt(startX + x, (i << 4) + y, startZ + z));
								obfuscate = true;
	...
	public static boolean areAjacentBlocksTransparent(ChunkInfo info, int x, int y, int z, int countdown)
	{
		...
		if(areAjacentBlocksTransparent(info, x, y + 1, z, countdown - 1)) return true;
		if(areAjacentBlocksTransparent(info, x, y - 1, z, countdown - 1)) return true;
		if(areAjacentBlocksTransparent(info, x + 1, y, z, countdown - 1)) return true;
		if(areAjacentBlocksTransparent(info, x - 1, y, z, countdown - 1)) return true;
		if(areAjacentBlocksTransparent(info, x, y, z + 1, countdown - 1)) return true;
		if(areAjacentBlocksTransparent(info, x, y, z - 1, countdown - 1)) return true;
	...
	public static boolean areAjacentBlocksBright(ChunkInfo info, int x, int y, int z, int countdown)
	{
		...
		if(areAjacentBlocksBright(info, x, y + 1, z, countdown - 1)) return true;
		if(areAjacentBlocksBright(info, x, y - 1, z, countdown - 1)) return true;
		if(areAjacentBlocksBright(info, x + 1, y, z, countdown - 1)) return true;
		if(areAjacentBlocksBright(info, x - 1, y, z, countdown - 1)) return true;
		if(areAjacentBlocksBright(info, x, y, z + 1, countdown - 1)) return true;
		if(areAjacentBlocksBright(info, x, y, z - 1, countdown - 1)) return true;
